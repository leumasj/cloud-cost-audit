...
const AI_VERIFY_COMMANDS = {
  ...
  ai_model_tier: 'grep -rn "model=" ./src --include=*.py | grep -v "mini\\|haiku"  # calls not using cheap tier',
  ai_no_caching: 'grep -rn "cache" ./src --include=*.py | wc -l  # cache references in codebase',
  ai_no_prompt_cache: 'grep -rn "cache_control" ./src --include=*.py | wc -l  # Anthropic prompt cache usage',
  ai_no_spend_cap: 'echo "Check: platform.openai.com/settings/organization/limits or console.anthropic.com/settings/limits"',
  ai_no_token_limits: 'grep -rn "max_tokens" ./src --include=*.py | wc -l  # calls with token limits set',
  ai_no_cost_alerts: 'echo "Check: platform.openai.com/settings/organization/billing/alerts"',
  ai_dev_hits_prod_api: 'grep -rn "OPENAI_API_KEY\\|ANTHROPIC_API_KEY" ./config ./.env* 2>/dev/null  # check for separate dev/prod keys',
    ...
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,16,0.85)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--border)", padding: "0 24px", height: "58px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <a href="https://www.upwork.com/freelancers/~015c346a56b09a2a89" target="_blank" rel="noopener noreferrer"
          style={{ background: "transparent", border: "none", color: "var(--text-dim)", fontSize: "13px", fontWeight: 600, textDecoration: "none" }}>
          Hire on Upwork
        </a>
