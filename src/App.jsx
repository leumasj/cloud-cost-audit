...
const globalCss = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .check-card:hover { transform: translateX(4px); background: rgba(255,255,255,0.04); }
  input:focus { outline: none; border-color: var(--green) !important; box-shadow: 0 0 0 3px rgba(0,255,180,0.1) !important; }
  .modal-overlay { position: fixed; inset: 0; z-index: 200; background: rgba(0,0,0,0.85); backdrop-filter: blur(10px); display: flex; align-items: center; justifyContent: center; padding: 20px; animation: fadeIn 0.3s ease; }
...
export default function App() {
  ...
  const Nav = ({ showBack, onBack }) => (
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,16,0.85)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--border)", padding: "0 24px", height: "58px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <span style={{ fontSize: "12px", color: "var(--text-dim)" }}>Professional Cost Intelligence</span>
    </nav>
  );

  const SampleReportModal = () => (
    <div className="modal-overlay" onClick={() => setShowSample(false)}>
      <div className="glass" onClick={e => e.stopPropagation()} style={{ maxWidth: "700px", width: "100%", borderRadius: "20px", padding: "40px", maxHeight: "90vh", overflowY: "auto" }}>
        <h2 className="display" style={{ color: "#fff", fontSize: "32px", marginBottom: "10px" }}>Sample: TechFlow GmbH</h2>
        <p style={{ color: "var(--text-dim)", marginBottom: "30px" }}>Based on an $8,500 AWS Monthly Bill</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "30px" }}>
          <div style={{ padding: "20px", borderRadius: "12px", background: "var(--green-dim)", border: "1px solid var(--green-border)" }}>
            <p style={{ fontSize: "12px", fontWeight: 700, color: "var(--green)" }}>MONTHLY SAVINGS</p>
            <p className="display" style={{ fontSize: "28px", color: "#fff" }}>$1,850 – $3,400</p>
          </div>
          <div style={{ padding: "20px", borderRadius: "12px", background: "rgba(129, 140, 248, 0.1)", border: "1px solid rgba(129, 140, 248, 0.2)" }}>
            <p style={{ fontSize: "12px", fontWeight: 700, color: "#818cf8" }}>WASTE RATE</p>
            <p className="display" style={{ fontSize: "28px", color: "#fff" }}>~32%</p>
          </div>
        </div>
        <button className="glow-btn" onClick={() => { setShowSample(false); goTo("intake"); }} style={{ width: "100%", padding: "16px", background: "var(--green)", borderRadius: "12px", color: "#000" }}>Run Your Own Audit →</button>
      </div>
    </div>
  );
  ...
  if (step === "intro") return (
    <div className="app">
      ...
      <ParticleBackground />
      {showSample && <SampleReportModal />}
      <Nav />
      <div style={{ position: "relative", zIndex: 1, maxWidth: "900px", margin: "0 auto", textAlign: "center", paddingTop: "100px" }}>
        <h1 className="display fade-up" style={{ fontSize: "72px", fontWeight: 800, color: "#fff", lineHeight: 1, marginBottom: "24px" }}>
          Stop the <span style={{ color: "var(--green)" }}>Cloud Leak.</span>
        </h1>
        <p className="fade-up" style={{ fontSize: "20px", color: "var(--text-dim)", maxWidth: "600px", margin: "0 auto 40px" }}>
          A professional 15-minute diagnostic to find exactly where your AWS/GCP bill is bleeding money.
        </p>
        <div className="fade-up" style={{ display: "flex", gap: "16px", justifyContent: "center" }}>
  ...
  if (step === "intake") return (
    <div className="app">
      ...
      <div className="fade-up" style={{ maxWidth: "500px", margin: "80px auto", padding: "0 24px" }}>
        <h2 className="display" style={{ fontSize: "36px", color: "#fff", marginBottom: "30px" }}>Audit Profile</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          <input placeholder="Company Name" value={companyName} onChange={e=>setCompanyName(e.target.value)} className="glass" style={{ width: "100%", padding: "16px", borderRadius: "12px", color: "#fff" }} />
          ...
          <input placeholder="Monthly Bill (USD)" type="number" value={monthlyBill} onChange={e=>setMonthlyBill(e.target.value)} className="glass" style={{ width: "100%", padding: "16px", borderRadius: "12px", color: "#fff" }} />
      ...
      <div className="app">
        ...
        <div style={{ maxWidth: "1000px", margin: "40px auto", padding: "0 24px", display: "grid", gridTemplateColumns: "1fr 320px", gap: "40px" }}>
              ...
              {section.checks.map(c => (
                <div key={c.id} className="glass check-card" onClick={() => toggle(c.id)} style={{ padding: "20px", borderRadius: "16px", border: checked[c.id] ? "1px solid var(--green)" : "1px solid var(--border)", background: checked[c.id] ? "var(--green-dim)" : "" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                    ...
                    <span style={{ fontSize: "12px", color: IMPACT_COLOR[c.impact] }}>{c.impact}</span>
                  </div>
                  <p style={{ fontSize: "14px", color: "var(--text-dim)" }}>{c.detail}</p>
                  {checked[c.id] && (
                    <div className="fade-up" style={{ marginTop: "15px", padding: "12px", background: "rgba(0,0,0,0.3)", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)" }}>
                      <p style={{ fontSize: "11px", color: "var(--green)", fontWeight: 800 }}>QUICK FIX:</p>
          ...
          <div className="glass" style={{ position: "sticky", top: "80px", padding: "24px", borderRadius: "20px", height: "fit-content" }}>
            <p style={{ fontSize: "11px", fontWeight: 800, color: "var(--text-dim)", marginBottom: "15px" }}>ESTIMATION STRATEGY</p>
            ...
            <p style={{ fontSize: "12px", color: "var(--text-dim)" }}>~{savPct}% of your bill</p>
          </div>
        </div>
      </div>
    );
  ...
  if (step === "report") return (
    <div className="app">
      ...
      <div className="fade-up" style={{ maxWidth: "800px", margin: "60px auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "15px", marginBottom: "40px" }}>
          ...
          <div className="glass" style={{ padding: "24px", borderRadius: "16px" }}>
            <p style={{ fontSize: "11px", color: "var(--text-dim)" }}>ANNUAL ROI</p>
            <p className="display" style={{ fontSize: "24px", color: "#818cf8" }}>${(savMin*12).toLocaleString()}+</p>
          </div>
          <div className="glass" style={{ padding: "24px", borderRadius: "16px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, rgba(0,255,180,0.1) 0%, rgba(99,102,241,0.1) 100%)", padding: "40px", borderRadius: "24px", border: "1px solid var(--green-border)", textAlign: "center" }}>
          <h3 className="display" style={{ fontSize: "24px", color: "#fff", marginBottom: "10px" }}>Get the PDF Implementation Guide</h3>
